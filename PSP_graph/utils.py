...
def load_data(dataset_str, type, task=None, shot=None):
    ...
    adj_dense_0_1 = (adj + sp.eye(adj.shape[0])).todense()
    adj_0_1 = adj + sp.eye(adj.shape[0])
    adj_0_1 = sparse_mx_to_torch_sparse_tensor(adj_0_1)
    ...
    if type == "pretrain":
        train_rate = len(labels)
        val_rate = 0
        idx_train = range(0, train_rate)
        idx_val = range(train_rate, train_rate + val_rate)
        idx_test = range(train_rate + val_rate, int(len(labels)))

        idx_train = torch.LongTensor(idx_train)
        idx_val = torch.LongTensor(idx_val)
        idx_test = torch.LongTensor(idx_test)
        labels = torch.LongTensor(labels)
        return adj_0_1, features, labels, idx_train, idx_val, idx_test
    else:
        train_rate = shot
        val_rate = shot
        trainset = []
        ...
        for i in range(task):
            ...
            for j in range(num_classes):
                train_0 = random.sample(index_dict[j], train_rate)
                remaining_0 = list(set(index_dict[j]) - set(train_0))
                val_0 = random.sample(remaining_0, val_rate)
                test_0 = list(set(index_dict[j]) - set(train_0) - set(val_0)) 
                _trainset += train_0
                _valset += val_0
                _testset += test_0
            #     print(len(_testset))  # 看每个类的数量分布
            # exit()
            trainset.append(_trainset)
            valset.append(_valset)
            testset.append(_testset)

        labels = torch.LongTensor(labels)
        if type == "prompt":
            return adj_0_1, features, labels, trainset, valset, testset
        elif type == "noprompt":
            return adj_0_1, features, labels, trainset, valset, testset
        else:
            print("ERROR!!!")
...
def compareloss(input,temperature):
    ...
    # print(input.size())   # torch.Size([3, 600, 126])
    ...
    # print(a.size(), positive.size(), negative.size())  # torch.Size([600, 126]) torch.Size([600, 126]) torch.Size([600, 126]) 每个图的表征
    return result.mean()
...
def link_prediction_mse_loss(scores, labels):
    mse_loss = torch.mean((scores - labels) ** 2)
    return mse_loss
...
def center_embedding(input,index,label_num,debug=False):
    ...
    c = c.scatter_add_(dim=0, index=index, src=input)  # 将input的值根据index label的位置相加
    c = c / _mean
    return c
